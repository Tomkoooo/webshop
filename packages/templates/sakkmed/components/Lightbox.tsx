...
export function DarkroomLightbox({ items, index, onClose, onIndexChange }: Props) {
  ...
  return (
    ...
    >
      <button type="button" className="absolute inset-0" aria-label="Bezárás" onClick={onClose} />
      <div className="relative z-10 flex max-h-[90vh] w-full max-w-5xl flex-col gap-3">
        <div className="flex items-center justify-between gap-4">
          <p id={titleId} className="sakkmed-kicker truncate text-accent">
            {item.caption || `Kép ${index + 1} / ${items.length}`}
          </p>
          <button
            type="button"
            onClick={onClose}
            className="sakkmed-focus inline-flex min-h-11 min-w-11 items-center justify-center rounded-full border border-foreground/20 text-foreground"
        ...
        <div className="flex justify-between gap-3">
          <button
            type="button"
            className="sakkmed-focus min-h-11 rounded-full border border-foreground/20 px-4 text-sm text-foreground"
